...
function addPost(postObj,type,socket,time){
	...
	np.save(function(){
		posts.find({},function(error,postArray){
			socket.broadcast.emit("updateTextPosts",[[np],np.type]);
		})
	});
}
...
app.get("/imgPage.html",function(req,res){
	res.sendfile("imgPage.html");
})
...
io.on('connection', function(socket){
		...
		socket.on("getData",function(data){
			posts.find({},function(error,postArray){
				socket.emit("updateTextPosts",[postArray,"noType"]);
			})
		});
